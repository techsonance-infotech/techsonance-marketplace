...
export function InnerSideBar({ links, style }: { links: Link[], style?: String | React.CSSProperties }) {
                            ...
                            <ul>
                                {
                                    section.list.map((link, linkIndex) => (
                                        <li key={linkIndex} className="mb-2">
                                            <Link to={link.path} className={`px-4 py-2 rounded-xl border-2  hover:text-blue-500 flex items-center  ${path == `${link.path}` ? 'font-bold bg-blue-50 border-2 text-blue-600  border-blue-300  ' : ' font-normal hover:bg-blue-50 hover:border-2 hover:text-blue-600  hover:border-blue-300 border-white '}`}>
