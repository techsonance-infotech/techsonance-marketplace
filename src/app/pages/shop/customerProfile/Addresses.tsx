...
import { createAddress, deleteAddress, updateAddress } from "../../../../features/UserSlice";

const addressTypes = ['home', 'work', 'other'];
const AddressForm = ({ user, addressId, operation, formToggle }: { user: UserProfile, addressId?: string, operation: 'edit' | 'add', formToggle: React.Dispatch<React.SetStateAction<boolean>> }) => {
    ...
    useEffect(() => {
        if (operation === 'edit' && existingAddress) {
            ...
            reset({ user_id: user?.user_id, address_for: 'home', is_default: false });
        }
    }, [addressId, operation, existingAddress, reset, user?.user_id]);
    const onSubmitHandler = (data: any) => {
        if (operation === 'edit') {
            ...
            console.log('edit', updatedAddress)
        } else {
const newAddress = {
                ...data,
                address_id: Date.now(),
                    ...
                    operation === 'edit' ? (
                        <form onSubmit={handleSubmit(onSubmitHandler)}>
                            ...
                            <div className="mt-6 flex gap-4">
                                ...
                                <button type="button" onClick={() => reset()} className="px-6 py-2 border-2 border-gray-300 rounded-lg hover:bg-gray-100">Reset</button>
                            </div>
                        </form>
                    ) : (
                        <> <form onSubmit={handleSubmit(onSubmitHandler)}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-lg font-medium text-gray-700 mb-2">Address Type</label>
                                ...
                                <div>
                                    <label className="block text-lg font-medium text-gray-700 mb-2">Address Line 1</label>
                                    <input type="text" {...register('address_line1')} className="mt-1 block w-full border-gray-300 rounded-md border-2 focus:ring-blue-500 focus:border-blue-500  py-2 px-6" />
                                </div>
                                <div>
    ...
    return (
        ...
        <>
            <section className="mb-[36vh] relative w-full">
                {user && formVisible && <AddressForm user={user} addressId={selectedAddressId} operation={formOperation} formToggle={setFormVisible} />}
                <header className="flex items-center justify-between w-full mb-6">
                    <h1 className="font-bold text-2xl    text-gray-900 ">
                        My Addresses
                    </h1>
                    <button className="px-6 py-2 border-2 text-primary bg-primary-foreground border-gray-800 rounded-lg cursor-pointer hover:border-gray-400  font-medium   " onClick={() => {
                ...
                {user?.addresses.length ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 ">
                        {user.addresses.map((address) => (
                            ...
                            <div key={address.address_id} className={`w-full border-2 rounded-lg px-4 pt-6 pb-4 text-gray-600 ${address.is_default ? 'border-blue-300 relative ' : 'border-gray-200'}`} >
                                {address.is_default && (
                                ...
                                <div className="w-full flex gap-8 justify-start items-start">
                                    ...
                                    <div className={`h-12 w-12 ${address.is_default ? 'bg-blue-300' : 'bg-gray-300'} rounded-3xl flex justify-center items-center  `}>
                                        <DynamicIcon name={`${address.address_for === 'home' ? 'house' : 'hotel'}`} size={32} className={` ${address.is_default ? 'text-blue-500' : 'text-gray-500'} `} />
                                    </div>
                                    <div>


                                        <h1 className="text-lg font-bold text-primary-foreground">{address.address_for.charAt(0).toUpperCase() + address.address_for.slice(1)} Address</h1>
                                ...
                                <div className="flex mt-4 items-end justify-start gap-12" >
                                    <button className=" py-2 font-bold text-blue-500  cursor-pointer  rounded-lg flex items-center gap-2" onClick={() => {
                                    ...
                                    <button className=" font-bold py-2 text-red-500  cursor-pointer  flex items-center gap-2" onClick={() => dispatch(deleteAddress(address.address_id))} >
                                        <Trash2 />
                                        Delete
                                    </button>
                                    {!address.is_default && (
                                        <button className=" py-2 cursor-pointer     rounded-lg flex items-center gap-2">
                                            SET AS DEFAULT
                                        </button>
                                    )}
                                </div>
