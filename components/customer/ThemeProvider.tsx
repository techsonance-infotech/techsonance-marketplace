...
export function getSafeContrastTextColor(bgColor: string, textColor: string): string {
...
export function ThemeProvider({ theme: initialTheme, children }: ThemeProviderProps) {
  ...
  const inlineStyles = useMemo(() => {
    ...
    if (correctedTheme.primary_color) {
      ...
      styles['--theme-primary-foreground'] = getSafeContrastTextColor(correctedTheme.primary_color, '#ffffff');
    }
    if (correctedTheme.secondary_color) styles['--theme-secondary'] = correctedTheme.secondary_color;
    if (correctedTheme.secondary_color) styles['--theme-accent'] = correctedTheme.secondary_color;
    if (correctedTheme.background_color) styles['--theme-background'] = correctedTheme.background_color;
    if (correctedTheme.text_color) styles['--theme-text'] = correctedTheme.text_color;

    // Apply main layout overrides locally
    if (correctedTheme.primary_color) {
      styles['--brand-primary'] = correctedTheme.primary_color;
      styles['--brand-primary-foreground'] = getSafeContrastTextColor(correctedTheme.primary_color, '#ffffff');
    }
    if (correctedTheme.secondary_color) styles['--brand-secondary'] = correctedTheme.secondary_color;
    if (correctedTheme.background_color) styles['--background'] = correctedTheme.background_color;
    if (correctedTheme.text_color) styles['--foreground'] = correctedTheme.text_color;
    if (correctedTheme.navbar_bg) styles['--navbar'] = correctedTheme.navbar_bg;
    if (correctedTheme.navbar_fg) styles['--navbar-foreground'] = correctedTheme.navbar_fg;
...
export function ThemeSection({ theme, className = '', children }: ThemeSectionProps) {
