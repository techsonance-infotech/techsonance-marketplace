...
import {
    CheckCircle2, ChevronLeft, ChevronRight, Star, Tag, X,
    Share2, Heart, Shield, Truck, RotateCcw, Banknote, FileSpreadsheet,
    Package, AlertCircle, Clock, Loader2, MessageSquareQuote, ChevronDown
} from 'lucide-react';
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="flex gap-4">
                    <div className="hidden lg:flex flex-col gap-3">
                        {[...Array(4)].map((_, i) => <SkeletonBox key={i} className="w-[72px] h-[72px]" />)}
                    </div>
                ...
                <div className="flex flex-col gap-5">
                    ...
                    <div className="flex gap-3 mt-2">
                        {[...Array(3)].map((_, i) => <SkeletonBox key={i} className="w-9 h-9 rounded-full" />)}
                    </div>
                    <div className="flex gap-3">
                        {[...Array(3)].map((_, i) => <SkeletonBox key={i} className="w-20 h-10 rounded-xl" />)}
                    </div>
...
type Action =
    ...
    | { type: ActionType.SET_PRODUCT_DATA; payload: { product: Product; variant?: Variant; images: ProductImage[]; activeImage?: string } }
    | { type: ActionType.SET_ACTIVE_VARIANT; payload: { variant: Variant; images: ProductImage[]; activeImage?: string } }
...
function reducer(state: State, action: Action): State {
    switch (action.type) {
        case ActionType.SET_MOUNTED: return { ...state, isMounted: action.payload };
        ...
        case ActionType.SET_ACTIVE_IMAGE: return { ...state, activeImage: action.payload };
        case ActionType.SET_ACTIVE_INDEX: return { ...state, activeIndex: action.payload };
        ...
        case ActionType.SET_ACTIVE_TAB: return { ...state, activeTab: action.payload };
        case ActionType.SET_QUANTITY: return { ...state, quantity: action.payload };
        default: return state;
    }
}

// ─── Main Page ────────────────────────────────────────────────────────────────
export default function ProductClient({ id, initialProduct }: { id: string, initialProduct: Product }) {
    ...
    const [state, dispatch] = useReducer(reducer, {
        ...
        activeVariant: initialProduct.variants?.[0],
        productImages: initialProduct.variants?.[0]?.images || [],
        activeImage: initialProduct.variants?.[0]?.images?.[0]?.image_url,
        activeIndex: 0,
    ...
    useEffect(() => { dispatch({ type: ActionType.SET_MOUNTED, payload: true }); }, []);
    ...
    useEffect(() => {
        const idx = state.productImages.findIndex(img => img.image_url === state.activeImage);
        if (idx !== -1) dispatch({ type: ActionType.SET_ACTIVE_INDEX, payload: idx });
    }, [state.activeImage, state.productImages]);

    const handleVariantChange = (variant: Variant) => {
        dispatch({
            type: ActionType.SET_ACTIVE_VARIANT,
            payload: { variant, images: variant.images, activeImage: variant.images[0]?.image_url }
        });
    };

    const basePrice = Number(state.activeVariant?.price) || 0;
    const discountPct = Number(state.product?.discount_percent) || 0;
    const originalMRP = discountPct > 0 ? Math.floor(basePrice / (1 - discountPct / 100)) : basePrice;

    let couponDiscount = 0;
    if (state.selectedCoupon) {
        if (state.selectedCoupon.discount_type === 'percentage') {
            couponDiscount = Math.floor(basePrice * (Number(state.selectedCoupon.discount_value) / 100));
            if (state.selectedCoupon.max_discount_amount && couponDiscount > Number(state.selectedCoupon.max_discount_amount)) {
        ...
        await AxiosAPI.post('/v1/coupon/validate', {
            ...
            .then((res: AxiosResponse) => {
                if (res.data.success !== true || res.status !== 201) {
                    ...
                    setTimeout(() => dispatch({ type: ActionType.SET_COUPON_MODAL_OPEN, payload: false }), 1500);
                } else {
    ...
    if (state.isLoading) return <ProductPageSkeleton />;
    ...
    return (
        <main className="min-h-screen bg-white">
            ...
            <nav className="hidden lg:flex items-center gap-2 max-w-7xl mx-auto px-8 pt-6 pb-2 text-xs text-gray-400 font-medium">
                ...
                <span className="hover:text-gray-700 cursor-pointer transition-colors capitalize">{state.product?.category?.name || 'Products'}</span>
                <ChevronRight size={12} />
                <span className="text-gray-800 font-semibold">{state.product?.name}</span>
            </nav>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
                        ...
                        {!isMobile && state.productImages.length > 1 && (
                            <div className="flex flex-col gap-3 lg:overflow-y-auto lg:h-0 lg:min-h-full [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                                {state.productImages.map((img, idx) => (
                                    <motion.button
                                        key={idx}
                                        onClick={() => { dispatch({ type: ActionType.SET_ACTIVE_IMAGE, payload: img.image_url }); dispatch({ type: ActionType.SET_ACTIVE_INDEX, payload: idx }); }}
                                        whileHover={{ scale: 1.04 }}
                                        whileTap={{ scale: 0.96 }}
                                        className={`shrink-0 w-[72px] h-[72px] rounded-xl overflow-hidden border-2 transition-all duration-200 ${state.activeImage === img.image_url
                                ...
                                <>
                                    <AnimatePresence mode="wait">
                                        <motion.img
                                            ...
                                            dragElastic={0.15}
                                            onDragEnd={(_, info) => {
                                                if (info.offset.x < -50 && state.activeIndex < state.productImages.length - 1) {
                                                    const n = state.activeIndex + 1;
                                                    dispatch({ type: ActionType.SET_ACTIVE_INDEX, payload: n });
                                                    dispatch({ type: ActionType.SET_ACTIVE_IMAGE, payload: state.productImages[n].image_url });
                                                } else if (info.offset.x > 50 && state.activeIndex > 0) {
                                                    const p = state.activeIndex - 1;
                                                    dispatch({ type: ActionType.SET_ACTIVE_INDEX, payload: p });
                                                    dispatch({ type: ActionType.SET_ACTIVE_IMAGE, payload: state.productImages[p].image_url });
                                                }
                                            }}
                                        />
                                    </AnimatePresence>
                                    ...
                                    {state.productImages.length > 1 && (
                                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
                                            {state.productImages.map((_, idx) => (
                                                <button
                                                    key={idx}
                                                    onClick={() => { dispatch({ type: ActionType.SET_ACTIVE_INDEX, payload: idx }); dispatch({ type: ActionType.SET_ACTIVE_IMAGE, payload: state.productImages[idx].image_url }); }}
                                    ...
                                    {state.activeIndex > 0 && (
                                        <button onClick={() => { const p = state.activeIndex - 1; dispatch({ type: ActionType.SET_ACTIVE_INDEX, payload: p }); dispatch({ type: ActionType.SET_ACTIVE_IMAGE, payload: state.productImages[p].image_url }); }}
                                    ...
                                    {state.activeIndex < state.productImages.length - 1 && (
                                        <button onClick={() => { const n = state.activeIndex + 1; dispatch({ type: ActionType.SET_ACTIVE_INDEX, payload: n }); dispatch({ type: ActionType.SET_ACTIVE_IMAGE, payload: state.productImages[n].image_url }); }}
                    ...
                    >
                        ...
                        >
                            ...
                            {totalReviews > 0 && (
                                <button
                                    onClick={() => dispatch({ type: ActionType.SET_ACTIVE_TAB, payload: 'reviews' })}
                                    className="flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 transition-colors"
                                >
                                    <StarRow rating={avgRating} />
                                    <span className="font-semibold text-gray-800">{(reviewsList.reduce((s, r) => s + r.rating, 0) / totalReviews).toFixed(1)}</span>
                                ...
                                {hasDiscount && (
                                    <>
                                        ...
                                        <span className="text-sm font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 px-2.5 py-0.5 rounded-full">
                                            {discountPct > 0 ? `${discountPct}% OFF` : `Save ₹${formatCurrency(totalSavings)}`}
                                        </span>
                                    </>
                                )}
                            </div>
                            <p className="text-xs text-gray-400 mt-1 font-medium uppercase tracking-wider">Inclusive of all taxes</p>

                            {/* Stock indicator */}
                            <div className="flex items-center gap-2 mt-3">
                                <span className={`w-2 h-2 rounded-full ${inStock ? 'bg-emerald-500' : 'bg-red-400'}`} />
                                <span className={`text-sm font-semibold ${inStock ? 'text-emerald-700' : 'text-red-500'}`}>
                                    ...
                                    {state.product.variants.map((variant) => (
                                        <button
                                            ...
                                            className={`group relative flex flex-col items-center rounded-xl border-2 overflow-hidden transition-all duration-200 px-2 pt-2 pb-1.5 min-w-[72px]
                                                ${state.activeVariant?.id === variant.id
                                                    ? 'border-gray-900 shadow-md'
                                                    : 'border-gray-200 hover:border-gray-400'
                                                }`}
                                        >
                                            {variant.images?.[0] && (
                                                <img src={variant.images[0].image_url} alt={variant.variant_name} className="w-12 h-12 object-contain" />
                                            )}
                            ...
                            {state.selectedCoupon ? (
                                <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-emerald-800 uppercase tracking-wide">{state.selectedCoupon.code} Applied</p>
                                            <p className="text-xs text-emerald-600">Extra ₹{formatCurrency(couponDiscount)} savings!</p>
                                        </div>
                                    </div>
                                    <button onClick={() => dispatch({ type: ActionType.SET_SELECTED_COUPON, payload: null })} className="p-1.5 hover:bg-red-50 rounded-lg transition-colors text-emerald-500 hover:text-red-500">
                                        <X size={15} />
                                    </button>
                                </div>
                            ) : (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="text-left">
                                            <p className="text-sm font-bold text-blue-800">Available Offers</p>
                                            <p className="text-xs text-blue-500">Tap to view & apply coupons</p>
                                        </div>
                                    </div>
                ...
                <div className="flex border-b border-gray-200 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    {(
                        [
                            ...
                            { key: 'reviews', label: `Customer Reviews${totalReviews > 0 ? ` (${totalReviews})` : ''}` },
                        ] as { key: Tab; label: string }[]
                    ).map(tab => (
                        <button
                            key={tab.key}
                            onClick={() => dispatch({ type: ActionType.SET_ACTIVE_TAB, payload: tab.key })}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                ...
                                {state.product?.features && state.product.features.length > 0 && (
                                    <div className="space-y-3">
                                        <h3 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-4">Key Features</h3>
