...
function reducer(state: State, action: Action): State {
    switch (action.type) {
        case ActionType.SET_TARGET_ITEM: return { ...state, targetItem: action.payload };
        case ActionType.SET_RATING: return { ...state, rating: action.payload, error: "" };
        case ActionType.SET_HOVER_RATING: return { ...state, hoverRating: action.payload };
        case ActionType.SET_REVIEW_TEXT: return { ...state, reviewText: action.payload };
        case ActionType.SET_SUBMITTING: return { ...state, isSubmitting: action.payload };
        case ActionType.SET_ERROR: return { ...state, error: action.payload, isSubmitting: false };
...
export default function WriteReviewPage() {
        ...
        const fetchOrderAndReview = async () => {
            ...
            if (orderRes?.data) {
                ...
                if (specificItem?.variant?.id && userId) {
                    ...
                    if (reviewRes?.data) {
                        dispatch({ type: ActionType.SET_RATING, payload: reviewRes.data.rating });
                        dispatch({ type: ActionType.SET_REVIEW_TEXT, payload: reviewRes.data.review });
                    }
                }
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Basic Validation
        if (state.rating === 0) {
            dispatch({ type: ActionType.SET_ERROR, payload: "Please select a star rating." });
            return;
        }
        if (state.reviewText.trim().length < 10) {
            dispatch({ type: ActionType.SET_ERROR, payload: "Please write at least 10 characters describing your experience." });
            return;
        }
        ...
        try {
            ...
            dispatch({ type: ActionType.SET_ERROR, payload: "Failed to submit review. Please try again later." });
        }
    };
    console.log('targetItem', state.targetItem);
    if (!state.targetItem) return <div className="p-8 text-center text-gray-500">Loading item details...</div>;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-6">
                <div className="flex gap-4 items-center">
                    ...
                    />
                    <div>
                        <p className="font-semibold text-gray-900 text-lg">{state.targetItem.variant.variant_name}</p>
            ...
            <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-200">
                    ...
                    <div className="flex gap-2">
                        {[1, 2, 3, 4, 5].map((starIndex) => (
                            <button
                                key={starIndex}
                                type="button"
                                onMouseEnter={() => dispatch({ type: ActionType.SET_HOVER_RATING, payload: starIndex })}
                                onMouseLeave={() => dispatch({ type: ActionType.SET_HOVER_RATING, payload: 0 })}
                                onClick={() => dispatch({ type: ActionType.SET_RATING, payload: starIndex })}
                                className="p-1 hover:scale-110 transition-transform focus:outline-none"
                            >
                                <Star
                                    size={36}
                                    className={`${(state.hoverRating || state.rating) >= starIndex
                ...
                <div className="mb-8">
                    ...
                    <textarea
                        ...
                        onChange={(e) => dispatch({ type: ActionType.SET_REVIEW_TEXT, payload: e.target.value })}
