...
type Action =
  ...
  | {
      ...
      payload: State["confirmModalConfig"];
    }
  | { type: AddressActionType.SET_CONFIRM_MODAL_OPEN; payload: boolean }
  | { type: AddressActionType.SET_PROCESSING; payload: boolean };

function reducer(state: State, action: Action): State {
  switch (action.type) {
    case AddressActionType.OPEN_MODAL:
      return {
        ...state,
        isModalOpen: true,
    ...
    case AddressActionType.CLOSE_MODAL:
      return { ...state, isModalOpen: false, selectedId: undefined };
    case AddressActionType.SET_ADDRESS_LIST:
      return { ...state, addressList: action.payload };
    case AddressActionType.SET_CONFIRM_MODAL:
      return {
        ...state,
        confirmModalConfig: action.payload,
        isConfirmModalOpen: true,
      };
    case AddressActionType.SET_CONFIRM_MODAL_OPEN:
      return { ...state, isConfirmModalOpen: action.payload };
    case AddressActionType.SET_PROCESSING:
...
export default function AddressesClient() {
  ...
  useEffect(() => {
    const fetchAddresses = async () => {
      if (user?.id && token) {
        ...
        dispatch({
          type: AddressActionType.SET_ADDRESS_LIST,
    ...
    dispatch({
      ...
      payload: {
        ...
        onConfirm: async () => {
          ...
          dispatch({
            type: AddressActionType.SET_ADDRESS_LIST,
            payload: state.addressList.filter((addr) => addr.id !== id),
          });
          dispatch({ type: AddressActionType.SET_PROCESSING, payload: false });
          dispatch({
            type: AddressActionType.SET_CONFIRM_MODAL_OPEN,
            payload: false,
          });
        },
      },
  ...
  const handleSetDefault = async (userId: string, id: string) => {
    if (token) {
      ...
      dispatch({
        type: AddressActionType.SET_ADDRESS_LIST,
        payload: state.addressList.map((addr) => ({
          ...addr,
  ...
  return (
    <section className="w-full max-w-[1200px] mx-auto py-4 md:py-8 lg:px-8 px-4 pb-24 font-sans">
      ...
      <AnimatePresence>
        {state.isModalOpen && user?.id && (
          <AddressModal
            ...
            onClose={() => dispatch({ type: AddressActionType.CLOSE_MODAL })}
          />
        )}
      </AnimatePresence>
      ...
      <ConfirmationModal
        isOpen={
          state.confirmModalConfig.actionType === "danger" &&
