...
const getStatusBadges = (statuses: string | string[]) => {
  ...
  const renderBadge = (status: string, index: number) => {
    switch (status) {
      case "pending":
        return <span key={index} className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-200 py-1 px-3 rounded-full text-theme-caption font-semibold">● Pending</span>;
      case "delivered":
        return <span key={index} className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200 py-1 px-3 rounded-full text-theme-caption font-semibold">● Delivered</span>;
      case "active":
        return <span key={index} className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-200 py-1 px-3 rounded-full text-theme-caption font-semibold">● Active</span>;
      case "cancelled":
        return <span key={index} className="inline-flex items-center gap-1 bg-red-50 text-red-700 border border-red-200 py-1 px-3 rounded-full text-theme-caption font-semibold capitalize">● {status}</span>;
      case "shipped":
        return <span key={index} className="inline-flex items-center gap-1 bg-violet-50 text-violet-700 border border-violet-200 py-1 px-3 rounded-full text-theme-caption font-semibold">● Shipped</span>;
      case "return":
      case "replacement":
        return <span key={index} className="inline-flex items-center gap-1 bg-purple-50 text-purple-700 border border-purple-200 py-1 px-3 rounded-full text-theme-caption font-semibold capitalize">● {status}</span>;
      default:
        return <span key={index} className="inline-flex items-center gap-1 bg-gray-100 text-gray-600 border border-gray-200 py-1 px-3 rounded-full text-theme-caption font-semibold capitalize">● {status}</span>;
    }
  };
...
const getPaymentBadge = (method: string, status: string) => {
  ...
  return (
    <span className={`inline-flex items-center py-1 px-3 rounded-full text-theme-caption font-semibold border ${isPaid ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-gray-100 text-gray-600 border-gray-200"}`}>
            {method || "N/A"}
        </span>);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-6">
                                ...
                                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex items-start justify-between hover:shadow-md transition-shadow">
                                    <div className="flex flex-col gap-1">
                                        <span className="text-theme-caption font-semibold text-gray-400 uppercase tracking-wider">Total Revenue</span>
                ...
                <TrendingUp size={20} className="text-emerald-500" />
                                ...
                                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex items-start justify-between hover:shadow-md transition-shadow">
                                    <div className="flex flex-col gap-1">
                                        <span className="text-theme-caption font-semibold text-gray-400 uppercase tracking-wider">Pending Orders</span>
                                        <span className="text-theme-h4 font-bold text-gray-800 mt-1">
                                            {formatNumber(pendingOrders)}
                                        </span>
                                        <span className="text-theme-caption text-amber-600 font-medium mt-1">Requires immediate shipping</span>
                                    </div>
                                ...
                                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex items-start justify-between hover:shadow-md transition-shadow">
                                    <div className="flex flex-col gap-1">
                                        <span className="text-theme-caption font-semibold text-gray-400 uppercase tracking-wider">Active Products</span>
                                        <span className="text-theme-h4 font-bold text-gray-800 mt-1">
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 my-6">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                <h2 className="font-bold text-theme-h6 text-gray-800">Revenue Overview</h2>
                                <p className="text-theme-caption text-gray-500">Last 30 Days</p>
                            </div>
                        </div>
                        {isLoadingChart ?
            ...
            <div className="w-full h-full flex items-center justify-center text-gray-400 text-theme-body-sm">
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 my-6">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="font-bold text-theme-h6 text-gray-800">Top Performing Products</h2>
                        </div>
              ...
              <div key={product.variant_id} className="flex items-center justify-between p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="font-semibold text-gray-800 text-theme-body-sm">{product.variant_name}</p>
                                                <p className="text-theme-caption text-gray-500">SKU: {product.sku}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-gray-800">₹{product.revenue.toLocaleString()}</p>
                                            <p className="text-theme-caption text-emerald-600 font-medium">{product.total_sold} units sold</p>
                                        </div>
                                    </div>
              ) :
              <div className="text-center py-6 text-gray-400 text-theme-body-sm">
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden my-6">
                        <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100">
                            <h2 className="font-bold text-theme-h6 text-gray-800">Recent Orders</h2>
                ...
                <button
                  ...
                  className="flex items-center gap-2 font-semibold text-theme-body-sm bg-purple-500 hover:bg-purple-600 text-white rounded-xl px-5 py-2.5 transition-colors shadow-sm disabled:opacity-50">
                                ...
                                <select name="" className='text-theme-body-sm border border-gray-200 bg-gray-50 rounded-xl px-3 py-2 text-gray-600 outline-none focus:border-blue-400 cursor-pointer transition-colors' id="" onChange={(e) => handleOrderFilter(e.target.value as OrderStatusType)}>
                  ...
                  className="flex items-center gap-1 text-theme-body-sm font-semibold text-blue-600 hover:text-blue-800 hover:underline transition-colors">
                    ...
                    <th key={header} className="p-4 text-theme-caption Rent-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">{header}</th>
                    )}
                                    </tr>
                                </thead>
                  ...
                  <tr>
                                            <td colSpan={10} className="py-16 text-center text-gray-400 text-theme-body-sm">
