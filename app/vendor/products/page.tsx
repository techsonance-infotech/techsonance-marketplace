...
const getCategoryOptions = async (setIsLoadingCategory: (loading: boolean) => void, setCategoryOptions: (options: { value: string; label: string }[]) => void, token: string) => {
                    ...
                    <TableBody className="divide-y divide-gray-100">
                        {isLoadingProducts ? (
                            ...
                            productList && Array.isArray(productList) && productList.map((item: Product, index: number) => {
                                ...
                                return (
                                    <TableRow key={item.id ?? index} className="hover:bg-gray-50 transition-colors" onClick={() => router.push(`/vendor/products/${item.id}/productVariants`)}>
                                        {/* Product Image + Name */}
