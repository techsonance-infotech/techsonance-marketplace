...
export default function Products() {
  ...
  const loadProduct = async () => {
    ...
    setTotalPages(
      Math.ceil((response?.data.total_products || 0) / itemsPerPage),
    );
    setIsLoadingProducts(false);
  };
  useEffect(() => {
  ...
  useEffect(() => {
    getCategoryOptions(setIsLoadingCategories, setCategoryOptions, token ?? "");
  }, [token]);
  useEffect(() => {
                  ...
                  >
                    ...
                    <TableCell className="px-4 py-3">
                      {item.variants && item.variants.length > 0 ? (
                        <span
                          className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-700 py-1.5 px-3 rounded-full transition-colors whitespace-nowrap"
                          title="View Variants"
                        >
                    ...
                    <TableCell className="px-4 py-3">
                      {stockQty !== undefined ? (
                        <span
                          className={`inline-flex items-center text-xs font-semibold py-1 px-3 rounded-full border ${
                            isLowStock
                    ...
                    <TableCell className="px-4 py-3 cursor-pointer">
                      <span
                        className={`flex text-sky-600 items-center gap-2 justify-center  text-xs font-semibold  px-3 rounded-lg transition-colors underline`}
                      >
                        View Variants
                        <MoveRight size={18} />
                      </span>
