...
export default function VendorOrderDetails() {
...

    {
        order?.items.map((item, index) => {
            ...
            const canCancel =
                !isCancelled &&
                ['pending', 'processing'].includes(order.items[index].order_status.toLowerCase());
            return (
                <div className="min-h-screen p-4 md:p-6 font-sans text-gray-800">
                    <div className="mx-auto">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                           ...
                           px-2 py-1 rounded-full w-fit">
                                    ...
                                    {canCancel && cancellingItemId !== item.productVariant.id && (
                                        <button
                                            onClick={() => setCancellingItemId(item.productVariant.id)}
                                            className="text-sm px-3 py-1.5 border border-red-300 
                       ...
                       transition-colors w-fit"
                                    ...
                                    {canCancel && cancellingItemId === item.productVariant.id && (
                                        <div className="flex flex-col gap-2 border border-red-200 
                          rounded-xl p-3 bg-red-50/30">
                                            <select
                                                value={cancelReason}
                         ...
                         px-3 py-2 outline-none"
                                            ...
                                            <div className="flex gap-2">
                                                <button
                                                    ...
                                                    onClick={() => handleCancelItem(item.productVariant.id)}
                                                    className="text-xs px-4 py-2 bg-red-600 text-white 
                           rounded-lg disabled:opacity-50"
                                                ...
                                                <button
                                                    ...
                                                    className="text-xs px-4 py-2 border border-gray-300 
                   ...
                   focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className={`text-sm font-medium px-2.5 py-1 rounded-md ${STATUS_CONFIG[order?.items[0]?.order_status as OrderStatus]?.className}`}>
                                                        {STATUS_CONFIG[order?.items[0]?.order_status as OrderStatus]?.label}
                                                    </span>
                                                    <button
