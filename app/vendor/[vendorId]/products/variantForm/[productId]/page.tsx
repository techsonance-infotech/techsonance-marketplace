...
const fetchProductMainDetails = async ({ productId, setProductDetails, token }: { productId: string, setProductDetails: (productDetails: { id: string; name: string; category: { id: string; name: string }; }) => void, token: string }) => {
...
export default function ProductVariantFormPage() {
    ...
    const [productDetails, setProductDetails] = useState<{ id: string; name: string; category: { id: string; name: string }; } | null>(null);
    ...
    return (
        <main className="min-h-screen py-8 px-4 w-full">
            <div className="mx-auto">
                <ProductVariantForm vendorId={vendorId} productDetails={productDetails ?? { id: "", name: "", category: { id: "", name: "" } }} productId={productId} warehouseOptions={warehouseOptions} />
            </div >
        </main >
    );
}