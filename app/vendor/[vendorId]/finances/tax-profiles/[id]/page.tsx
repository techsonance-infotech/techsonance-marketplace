...
export default function UnifiedTaxProfileFormPage() {
    ...
    const onSubmit = async (data: any) => {
        try {
            if (isEditMode) {
                await fetchUpdateTaxProfile(profileId, data, vendorId, token!);
            } else {
                await fetchCreateTaxProfile(data, token!);
            }
            router.push(`/vendor/${vendorId}/finances/tax-profiles`);
        } catch (error) {
    ...
    return (
        <section className="w-full  px-1">
            <header className="flex justify-between items-center my-6">
                ...
                <Link href={`/vendor/${vendorId}/finances/tax-profiles`} className="flex items-center gap-2 text-sm bg-white border border-gray-200 text-gray-700 rounded-xl px-5 py-2.5 shadow-sm hover:bg-gray-50">
