...
import {
    ShieldCheck, Tag, Box, ChevronRight, Loader2,
    AlertCircle, Search, Filter, ArrowLeft, Layers
} from 'lucide-react';
...
const TYPE_PALETTE: Record<string, { border: string; badge: string; icon: string }> = {
    warranty: { border: 'border-l-blue-500', badge: 'bg-blue-50 text-blue-700', icon: '🛡️' },
    guarantee: { border: 'border-l-emerald-500', badge: 'bg-emerald-50 text-emerald-700', icon: '✅' },
    exchange_only: { border: 'border-l-amber-500', badge: 'bg-amber-50 text-amber-700', icon: '🔄' },
    no_return: { border: 'border-l-red-500', badge: 'bg-red-50 text-red-700', icon: '🚫' },
    extended_support: { border: 'border-l-purple-500', badge: 'bg-purple-50 text-purple-700', icon: '🔧' },
    none: { border: 'border-l-gray-300', badge: 'bg-gray-50 text-gray-600', icon: '➖' },
};
...
export default function PolicyCoveragePage() {
    ...
    const [filterScope, setFilterScope] = useState<'all' | 'categories' | 'products'>('all');
...
useEffect(() => {
    ...
    const policyTypes = ['all', ...Array.from(new Set(data.map(d => d.policy.policy_type)))];

    const filtered = data.filter(item => {
        const matchesSearch =
            item.policy.policy_name.toLowerCase().includes(search.toLowerCase()) ||
            item.categories.some(c => c.name.toLowerCase().includes(search.toLowerCase())) ||
            item.products.some(p => p.name.toLowerCase().includes(search.toLowerCase()));

        const matchesType = filterType === 'all' || item.policy.policy_type === filterType;
    ...
    const unassigned = data.filter(d => d.categories.length === 0 && d.products.length === 0).length;
    ...
    return (
        <main className="w-full   mx-auto py-6 px-1">
            ...
            <div className="mb-6">
                <Link
                    href={`/vendor/${vendorId}/configDocuments`}
                ...
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">Policy Coverage Map</h1>
                        <p className="text-sm text-gray-500">See which categories and products each policy covers</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                {[
                    { label: 'Total Policies', value: data.length, icon: <ShieldCheck size={16} />, color: 'blue' },
                    { label: 'Category Links', value: totalCategories, icon: <Tag size={16} />, color: 'indigo' },
                    { label: 'Product Overrides', value: totalProducts, icon: <Box size={16} />, color: 'orange' },
                    { label: 'Unassigned', value: unassigned, icon: <AlertCircle size={16} />, color: 'gray' },
                ].map(s => (
                    ...
                    <div className="flex border border-gray-200 rounded-xl overflow-hidden bg-gray-50">
                        {(['all', 'categories', 'products'] as const).map(s => (
                            <button
                                ...
                                className={`px-3 py-2 text-xs font-medium capitalize transition-colors ${filterScope === s
            ...
            {filtered.length === 0 ? (
                <div className="bg-white border border-gray-200 rounded-2xl py-16 text-center text-gray-400 shadow-sm">
                    ...
                    <Link href={`/vendor/${vendorId}/configDocuments/assignments`}
                ...
                <div className="space-y-4">
                    {filtered.map(item => {
                        const palette = TYPE_PALETTE[item.policy.policy_type] ?? TYPE_PALETTE.none;
                        return (
                            <div
                                key={item.policy.id}
                                ...
                                <div className="p-5 flex flex-col sm:flex-row sm:items-center gap-3 border-b border-gray-100">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex flex-wrap items-center gap-2 mb-1">
                                            <h3 className="font-semibold text-gray-900 text-base">{item.policy.policy_name}</h3>
                                            <TypeBadge type={item.policy.policy_type} />
                                            {!item.policy.is_active && (
                                                <span className="px-2 py-0.5 text-xs font-bold bg-red-50 text-red-600 rounded-full">Inactive</span>
                                            )}
                                            {item.policy.generates_document && (
                                                <span className="px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-600 rounded-full">📄 Generates PDF</span>
                                            )}
                                        </div>
                                        {item.policy.duration_value && (
                                            <p className="text-xs text-gray-500">
                                                Duration: {item.policy.duration_value} {item.policy.duration_unit}
                                            </p>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        <span className="text-xs text-gray-500">
                                            {item.categories.length} {item.categories.length === 1 ? 'category' : 'categories'} · {item.products.length} {item.products.length === 1 ? 'product' : 'products'}
                                        </span>
                                        <Link
                                            href={`/vendor/${vendorId}/configDocuments/coverage/${item.policy.id}`}
                                    ...
                                    <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {item.categories.map(cat => (
                                                        ...
                                                        >
                                                            ...
                                                            {cat.name}
                                                            {cat.priority > 1 && (
                                                                <span className="ml-1 bg-indigo-200 text-indigo-800 px-1 rounded text-[10px]">p{cat.priority}</span>
                                                            )}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        <div>
                                            ...
                                            {item.products.length === 0 ? (
                                                <p className="text-xs text-gray-400 italic">No product overrides</p>
                                            ) : (
                                                <div className="flex flex-wrap gap-2">
