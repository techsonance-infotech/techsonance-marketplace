...
export default function AudiencesPage() {
  ...
  const triggerRecalculate = async (id: string, e: React.MouseEvent) => {
    ...
    setRecalculating(id);
    try {
      const res = await AxiosAPI.post(`/v1/audiences/${id}/recalculate`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const totalMembers = Array.isArray(segments) ? segments.reduce((s, seg) => s + (seg.member_count ?? 0), 0) : 0;
    ...
    <div className="p-6 w-full mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Segments</p>
          <p className="text-2xl font-bold text-gray-800">{loading ? "—" : segments.length}</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Total Members</p>
          <p className="text-2xl font-bold text-gray-800">{loading ? "—" : totalMembers.toLocaleString()}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {Array.isArray(segments) && segments.map((seg) => (
            <div
              key={seg.id}
              onClick={() => router.push(`/vendor/marketing/audiences/form?segmentId=${seg.id}`)}
            ...
            >
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{seg.name}</p>
                  <p className="text-sm text-gray-500 truncate">{seg.description ?? `${seg.criteria.length} criteria · ${seg.criteria_operator}`}</p>
                </div>
              </div>

              <div className="flex items-center gap-6 flex-shrink-0">
                <div className="text-right hidden sm:block">
                  <p className="text-lg font-bold text-gray-800">{(seg.member_count ?? 0).toLocaleString()}</p>
                ...
                <div className="text-right hidden md:block">
                  ...
                  <p className="text-xs font-medium text-gray-600">
                    {seg.last_recalculated_at
                      ? new Date(seg.last_recalculated_at).toLocaleDateString("en-IN", { day: "numeric", month: "short" })
                      : "Never"}
                  </p>
                </div>
                <button
                ...
                >
                  <RefreshCw size={16} className={recalculating === seg.id ? "animate-spin" : ""} />
                </button>
