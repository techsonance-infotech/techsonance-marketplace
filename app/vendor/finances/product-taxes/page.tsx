...
import {
  fetchTaxSlabs,
  fetchAssignProductTax,
  fetchProductTaxMappings,
  fetchBulkAssignProductTax,
...
export default function ProductTaxMappingPage() {
  ...
  const [availableRates, setAvailableRates] = useState<
    { id: string; slab_name: string; total_rate: number }[]
  >([]);
    ...
    <main className="w-full px-1">
      ...
      <header className="flex justify-between items-center my-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            className={`flex items-center gap-2 font-semibold text-sm rounded-xl px-5 py-2.5 transition-colors shadow-sm ${selectedProductIds.length > 0 ? "bg-blue-500 hover:bg-blue-600 text-white" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
            ...
            <tr className="bg-gray-50 border-b border-gray-200 text-left">
              <th className="p-4 w-10">
                <input
                  ...
                  checked={
                    productTaxes &&
                    selectedProductIds.length === productTaxes.length &&
            ...
            {loading ? (
              ...
              productTaxes.map((item) => (
                ...
                >
                  ...
                  <td className="p-4">
                    <Link
                      href={`/vendor/products/${item.id}`}
                      className="font-semibold text-blue-600 hover:underline line-clamp-2 text-sm text-wrap max-w-24"
                    >
                      {item.product_name}
                    </Link>
                  </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleAssignTax} className="p-6">
              <label className="text-sm font-semibold text-gray-700 block mb-2">
                Select a Tax Rate to apply to this product:
              </label>
              <select
                required
              ...
              {availableRates.length === 0 && (
                <p className="text-xs text-amber-600 mt-2">
                  ...
                  <Link
                    href={`/vendor/finances/tax-rates/new`}
              ...
              <div className="flex justify-end gap-3 mt-8">
                ...
                <button
                  disabled={saving || availableRates.length === 0}
                  type="submit"
                  className="px-5 py-2.5 text-sm font-semibold text-white bg-blue-500 hover:bg-blue-600 rounded-xl shadow-sm transition-colors disabled:opacity-50"
                >
