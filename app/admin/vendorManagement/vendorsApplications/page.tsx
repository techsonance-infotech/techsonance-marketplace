      ...
      setVendorApplications((prev) =>
        prev.map((app) =>
          app.id === vendorId
            ? {
                ...app,
                company: { ...app.company, company_status: "approved" },
      ...
      setVendorApplications((prev) =>
        prev.map((app) =>
          app.id === vendorId
            ? {
                ...app,
                company: { ...app.company, company_status: "rejected" },
      ...
      <div>
        {vendorApplications.map((application, index) => {
          ...
          return (
            <section
              key={index}
              className="border-2 border-gray-400 my-6 rounded-xl"
            >
              {/* ── Header ── */}
              <div
                  ...
                  <span className="flex gap-6 justify-between items-baseline">
                    <div className="flex flex-1 flex-col gap-6 border-2 border-gray-300 rounded-xl mt-6 px-8 py-4">
                      ...
                      <button
                        ...
                        className="cursor-pointer rounded-lg font-medium text-white bg-black py-2 px-4"
                      >
                        Verify & Approve Vendor
                      </button>
                      <button
                        onClick={() => onReject(vendor.id)}
                        className="cursor-pointer rounded-lg font-medium bg-red-50 text-red-500 py-2 px-4"
                      >
                        Reject
                      </button>
                    </div>
