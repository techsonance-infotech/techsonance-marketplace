...
template_label: string;
  ...
  const fetchData = async (setTemplates: React.Dispatch<React.SetStateAction<Template[]>>, setIsLoading: React.Dispatch<React.SetStateAction<boolean>>, token: string | null) => {
            try {
                setIsLoading(true);
                if (!token) {   
...
const [previewUrl, setPreviewUrl] = useState<string | null>(null);
    ...
    const handleDelete =async (id: string) => {
        token && setConfirmModalConfig({
            ...
            message: "This action cannot be undone. Are you sure you want to delete this template?",
            actionType: "danger",
    ...
    return (
        <>
            ...
            <main className="w-full px-1 py-4">
                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center my-6 gap-4">
                    <div className="flex items-center gap-2 text-gray-700">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-800">PDF Templates</h1>
                            <p className="text-xs text-gray-500 mt-1">Manage agreement and policy PDFs for vendors.</p>
                        </div>
                    </div>
                    ...
                    <table className="w-full table-auto min-w-[900px] border-collapse">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200 text-left">
                                <th className="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Template ID</th>
                                <th className="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Label</th>
                                <th className="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Description</th>
                                <th className="p-4 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">Document</th>
                            ...
                            { templates && templates.length === 0 ? (
                                ...
                                templates &&  templates.map((template) => (
                                    <tr key={template.id} className="hover:bg-gray-50 transition-colors group">
                                        <td className="p-4 font-medium text-gray-800">{template.template_name}</td>
                                        <td className="p-4 text-sm text-gray-800">{template.template_label}</td>
                                        <td className="p-4 text-sm text-gray-500 max-w-[250px] truncate" title={template.description}>
                                            {template.description}
                                        </td>
                                        <td className="p-4 text-sm text-blue-500 underline cursor-pointer"><button 
                                        ...
                                        <td className="p-4">
                                            <div className="flex items-center gap-3">
                                                <Link href={`templates/templateForm?id=${template.id}`} className="text-gray-500 hover:text-blue-600 transition-colors">
                                                    <Edit size={18} />
                                                </Link>
                                                <button className="text-gray-500 hover:text-red-600 transition-colors" onClick={() => {handleDelete(template.id)}}>
                                                    <Trash2 size={18} />
                                                </button>
                                            </div>
                                        </td>
            ...
            <ConfirmationModal  title={confirmModalConfig.title} message={confirmModalConfig.message} actionType={confirmModalConfig.actionType} confirmText={confirmModalConfig.confirmText} onConfirm={confirmModalConfig.onConfirm} onClose={() => setIsConfirmModalOpen(false)} isOpen={isConfirmModalOpen}
