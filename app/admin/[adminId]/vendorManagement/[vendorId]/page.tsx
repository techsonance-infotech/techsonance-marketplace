...
import {
    ArrowLeft, Store, User, Mail, Phone, Calendar,
    ShoppingBag, DollarSign, ShieldAlert, ShieldCheck,
    Globe, Briefcase, CheckCircle2, XCircle, Key,
    Clock, CalendarDays, Power, AlertOctagon, FileText, ExternalLink,
    Activity
} from "lucide-react";
...
export default function VendorDetailPage() {
            ...
            try {
                ...
                setVendorData({
                    ...
                    owner: {
                        ...
                        first_name: apiData.owner.store_owner_first_name || apiData.owner.user?.first_name || "N/A",
                        last_name: apiData.owner.store_owner_last_name || apiData.owner.user?.last_name || "",
                    ...
                    license: {
                        ...
                        // is_trial: false,
                        is_trial: true,
                        trial_ends_at: new Date(new Date(apiData.company.created_at).getTime() + 14 * 24 * 60 * 60 * 1000).toISOString(),
                        valid_from: apiData.company.created_at,
                        expires_at: new Date(new Date(apiData.company.created_at).getTime() + 365 * 24 * 60 * 60 * 1000).toISOString(),
                    },
                    stats: {
    ...
    const handleSuspendClick = () => {
        ...
        setModalConfig({
            ...
            message: "This will immediately hide the vendor's storefront and prevent them from logging in. This action can be reversed later.",
            actionType: "suspend",
    ...
    const handleActivateClick = () => {
        ...
        setModalConfig({
            ...
            message: "This will approve the vendor's KYC documents and grant them full access of storefront.",
            actionType: "approve",
    ...
    if (!vendorData) return (
        <div className="p-10 text-center text-red-500">Failed to load vendor data. Please try again.</div>
    );
            ...
            {isPending && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-amber-800">Verification Pending</h2>
                            <p className="text-sm text-amber-700 mt-1">
                                This vendor has applied to join the marketplace. Review their company details and KYC documents below before granting access.
                            </p>
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
                                <span className="capitalize">{vendorData.company.company_name}</span>
                            ...
                            <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-gray-500">
                                ...
                                <span className="flex items-center gap-1.5">
                                    <Calendar size={14} /> Registered {new Date(vendorData.company.created_at).toLocaleDateString()}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-gray-100">
                    <div className="bg-gray-50 p-4 rounded-xl">
                        ...
                        <p className="text-2xl font-bold text-gray-800">₹{vendorData.stats.total_revenue.toLocaleString()}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-xl">
                        <p className="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1">Total Orders</p>
                        <p className="text-2xl font-bold text-gray-800">{vendorData.stats.total_orders}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-xl">
                        <p className="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1">Catalog Size</p>
                        <p className="text-2xl font-bold text-gray-800">{vendorData.stats.active_products}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-xl">
                        <p className="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-1">Customers</p>
                        <p className="text-2xl font-bold text-gray-800">{vendorData.stats.total_customers}</p>
                    </div>
                </div>
            </div>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <p className="font-medium text-gray-800 capitalize">{vendorData.company.company_name}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Business Structure</p>
                                    <p className="font-medium text-gray-800 capitalize">{vendorData.company.company_structure.replace('_', ' ')}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Status</p>
                                    <p className="font-medium text-gray-800 capitalize">{vendorData.company.company_status}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <p className="font-medium text-gray-800 capitalize">{vendorData.owner.first_name} {vendorData.owner.last_name}</p>
                                </div>
                                <div>
                                    ...
                                    <div className="flex flex-col gap-1.5 mt-1">
                                        <p className="text-sm font-medium text-gray-800 flex items-center gap-2 truncate">
                                            <Mail size={14} className="text-gray-400 shrink-0" /> {vendorData.owner.email}
                                        </p>
                                        <p className="text-sm font-medium text-gray-800 flex items-center gap-2">
                                            <Phone size={14} className="text-gray-400 shrink-0" /> {vendorData.owner.phone_number}
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                <Key size={18} className="text-indigo-500" /> License & Subscription
                            </h3>
                        ...
                        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div>
                                ...
                                <p className="text-xl font-bold text-gray-800">{vendorData.license.plan_name}</p>

                                {vendorData.license.is_trial && vendorData.license.trial_ends_at && (
                                    <div className="mt-4 p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                                        <p className="text-xs font-semibold text-indigo-800 mb-2">Trial Ends in {calculateTrialDays(vendorData.license.trial_ends_at)} Days</p>
                                        <div className="w-full bg-indigo-200 rounded-full h-1.5">
                                            <div className="bg-indigo-500 h-1.5 rounded-full" style={{ width: '45%' }}></div>
                                        </div>
                                        <p className="text-xs text-indigo-600 mt-2">{new Date(vendorData.license.trial_ends_at).toLocaleDateString()}</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <p className="text-sm text-gray-500 mb-1 flex items-center gap-1.5"><CalendarDays size={14} /> License Issued</p>
                                    <p className="font-medium text-gray-800">{new Date(vendorData.license.valid_from).toLocaleDateString("en-GB", { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1 flex items-center gap-1.5"><AlertOctagon size={14} /> License Expiry</p>
                                    <p className="font-medium text-gray-800">{new Date(vendorData.license.expires_at).toLocaleDateString("en-GB", { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-3 justify-center border-l border-gray-100 md:pl-8">
                                {vendorData.license.is_trial ? (
                                    <>
                                        <button className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-sm cursor-pointer" onClick={handleActivateClick} >
                                            <CheckCircle2 size={16} /> Activate Full License
                                        </button>
                                        <button className="w-full flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors cursor-pointer" onClick={handleExtendTrial}>
                                    ...
                                    <button className="w-full flex items-center justify-center gap-2 bg-white border border-red-200 text-red-600 hover:bg-red-50 px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors cursor-pointer" >
                                        <Power size={16} onClick={handleDeactivateClick} /> Deactivate License
                                    </button>
                                )}

                                <button className="w-full flex items-center justify-center gap-2 bg-red-50 text-red-700 hover:bg-red-100 px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors mt-2 cursor-pointer" onClick={handleSuspendClick} >
