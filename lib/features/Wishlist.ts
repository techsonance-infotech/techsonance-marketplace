...
const saveWishlistToLocalStorage = (
  ...
  try {
    localStorage.setItem(
      WISHLIST_KEY,
      JSON.stringify({ wishlistId, wishItems }),
    );
  } catch (e) {}
};
...
const loadWishlistFromLocalOrServer = async (): Promise<
  Omit<WishlistState, "loading" | "error">
> => {
  if (!isClient) return { wishlistId: "", wishItems: [] };
  const token = authToken();
  const companyDomain = await getCompanyDomain();
  let localFallback: Omit<WishlistState, "loading" | "error"> = {
    wishlistId: "",
    wishItems: [],
  };
  ...
  try {
    ...
    if (
      serializedWishlist &&
      serializedWishlist !== "undefined" &&
      serializedWishlist !== "null"
    ) {
      const parsed = JSON.parse(serializedWishlist);
      if (parsed && Array.isArray(parsed.wishItems)) {
        localFallback = {
          ...parsed,
          wishItems: parsed.wishItems as WishlistItem[],
        };
      }
    }
    ...
    if (customerId && companyDomain && token) {
      ...
      if (
        ...
        if (serverData.length > 0) {
          const wishItems: WishlistItem[] = serverData.map(
            (item: WishlistServerResponse) => ({
              id: item.id,
