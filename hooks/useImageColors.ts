...
function findClosestPaletteColor(rDom: number, gDom: number, bDom: number, palette: string[]): string {
  ...
  for (const color of palette) {
    try {
      ...
      const distance = Math.sqrt(Math.pow(r - rDom, 2) + Math.pow(g - gDom, 2) + Math.pow(b - bDom, 2));
...
// ── Module-level cache: prevents re-running canvas extraction on every re-render ──
const colorCache = new Map<string, { bg: string; solidBg: string; dominant: string }>();
const pendingExtractions = new Map<string, Promise<{ bg: string; solidBg: string; dominant: string }>>();
...
function extractColors(imageUrl: string, options?: UseImageColorsOptions): Promise<{ bg: string; solidBg: string; dominant: string }> {
  const cacheKey = `${imageUrl}|${options?.mode ?? ''}|${options?.fallbackColor ?? ''}`;

  if (colorCache.has(cacheKey)) return Promise.resolve(colorCache.get(cacheKey)!);
  if (pendingExtractions.has(cacheKey)) return pendingExtractions.get(cacheKey)!;

  const promise = new Promise<{ bg: string; solidBg: string; dominant: string }>((resolve) => {
    const img = new Image();
    img.crossOrigin = 'Anonymous';
    img.src = imageUrl;

    img.onerror = () => { pendingExtractions.delete(cacheKey); resolve(FALLBACK_RESULT); };

    img.onload = () => {
      try {
        const canvas = document.createElement('canvas');
        const ctx = canvas.getContext('2d');
        if (!ctx) { resolve(FALLBACK_RESULT); return; }
        ...
        if (ctx1x1) {
          ...
          rDom = domData[0]; gDom = domData[1]; bDom = domData[2]; aDom = domData[3];
        }

        const brightness = 0.299 * rDom + 0.587 * gDom + 0.114 * bDom;
        const isLightMode = options?.mode === ColorMode.LIGHT || (options?.mode !== ColorMode.DARK && brightness > 128);
        let activeFallback = options?.fallbackColor || (isLightMode ? IMAGE_COLOR_DEFAULTS.bgLight : IMAGE_COLOR_DEFAULTS.bgDark);
        ...
        if (activeFallback.startsWith('rgb')) {
          ...
          if (matches && matches.length >= 3) fallbackRgb = [parseInt(matches[0]), parseInt(matches[1]), parseInt(matches[2])];
        } else if (activeFallback.startsWith('#')) {
          try { fallbackRgb = hexToRgb(activeFallback); } catch { /* ignore */ }
        }

        const resolveColor = (r: number, g: number, b: number, a: number) => {
          return `rgb(${Math.round(r * a + fallbackRgb[0] * (1 - a))}, ${Math.round(g * a + fallbackRgb[1] * (1 - a))}, ${Math.round(b * a + fallbackRgb[2] * (1 - a))})`;
        };
        ...
        const parseRgb = (s: string): [number, number, number] => {
          const m = s.match(/\d+/g); return m ? [+m[0], +m[1], +m[2]] : fallbackRgb;
        };
        const [trR, trG, trB] = parseRgb(topColorStr);
        const [brR, brG, brB] = parseRgb(bottomColorStr);
        const solidBgColor = `rgb(${Math.round((trR + brR) / 2)}, ${Math.round((trG + brG) / 2)}, ${Math.round((trB + brB) / 2)})`;
        ...
        if (isTransparentBg) {
          ...
          if (maxVariance < 15) finalBg = solidBgColor;
          else if (diffV >= diffH) finalBg = `linear-gradient(to bottom, ${topColorStr}, ${bottomColorStr})`;
          else finalBg = `linear-gradient(to right, ${leftColorStr}, ${rightColorStr})`;
        }
...
export function useImageColors(imageUrl: string | undefined, options?: UseImageColorsOptions) {
  ...
  // Seed from cache immediately — no flash of fallback color on second render
  const [colors, setColors] = useState<{ bg: string; solidBg: string; dominant: string }>(() => {
    if (imageUrl && colorCache.has(cacheKey)) return colorCache.get(cacheKey)!;
  ...
  useEffect(() => {
    if (!imageUrl) return;
    if (colorCache.has(cacheKey)) { setColors(colorCache.get(cacheKey)!); return; }
    extractColors(imageUrl, options).then((result) => {
